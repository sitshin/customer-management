...
export default function SendSMSPage() {
  ...
  // 👉 페이지네이션 관련 상태
  const [currentPage, setCurrentPage] = useState(1) // 수정된 부분
  const ITEMS_PER_PAGE = 10 // 수정된 부분
  ...
  const fetchCustomers = async () => {
    let query = supabase
      ...
      .order('customer_amount', { ascending: false })
    if (minAmount) {
    ...
    if (error) {
      ...
      setCurrentPage(1) // 페이지 초기화 // 수정된 부분
    }
  }
